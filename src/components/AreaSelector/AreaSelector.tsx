...
const Area = ({area, selectArea}) => (
  <div className={css.area + (preferenceStore.selectedArea === area.id ? ' ' + css.selected : '')}>
    <button
    ...
    return (
      <PageContainer title={<Text id="selectArea" />}>
        <div className={css.container}>
          {specialAreas.map(area => <Area key={area.id} selectArea={this.selectArea} area={area} />)}
          {sortBy(dataStore.areas.data, 'name').map(area => <Area key={area.id} selectArea={this.selectArea} area={area} />)}
        </div>
      </PageContainer>
    )
  }
